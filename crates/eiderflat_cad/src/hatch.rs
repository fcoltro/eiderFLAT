...
fn is_closed_poly(pc: &eiderflat_geometry::PolyCurve) -> bool {
...
fn region_edges(boundary: &[Curve], holes: &[Vec<Curve>]) -> Vec<(P, P)> {
    ...
    let mut add_loop = |segs: &[Curve]| {
        for seg in segs {
            ...
            let pts = tessellate_curve(seg, (diag * 0.02).max(1e-4));
...
fn region_bbox(boundary: &[Curve]) -> (f64, f64, f64, f64) {
    let mut bb: Option<eiderflat_geometry::BoundingBox> = None;
    for c in boundary {
    ...
    while changed {
        ...
        'pairs: for i in 0..regs.len() {
            for j in (i + 1)..regs.len() {
                if !intersection(&regs[i], &regs[j]).outer.is_empty() {
                    regs[i] = union(&regs[i], &regs[j]);
                    regs.remove(j);
                    changed = true;
                    break 'pairs;
                }
