    ...
    pub fn apply_corner_action(&mut self) {
        ...
        if is_poly {
            ...
            for i in idx.into_iter().rev() {
                match kind {
                    CornerKind::Fillet => {
                        eiderflat_cad::edit::fillet_poly_corner(&mut self.document, g.a, i, size);
                    }
                    CornerKind::Chamfer => {
                        eiderflat_cad::edit::chamfer_poly_corner(&mut self.document, g.a, i, size);
                    }
                }
            }
        } else {
            for c in &group {
                match kind {
                    ...
                    CornerKind::Chamfer => {
                        eiderflat_cad::edit::chamfer(&mut self.document, c.a, c.b, size, size);
                    }
                }
            }
